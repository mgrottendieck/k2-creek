...
macro_rules! filename_by_type {
    ($xml_type: ident) => {
        FILENAMES.get(&$xml_type).expect("No matching file type found in known FILENAMES")
    }
}
...
pub fn dump_egk_data_to_files(resp: &K2Response) {
    ...
    write_string_to_file(&create_mfefgdo_xml_string(&resp.iccsn), filename_by_type!(EgkMFEFGDO));
