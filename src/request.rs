...
fn handle_response_failure_cases(resp: &mut ::reqwest::Response) -> K2Response {
    ...
    let resp_body = resp.text().expect(&format!("Unable to read K2 response body. Status: {:?}", status));
    match status {
        StatusCode::Ok => {
            panic!("Response status was OK but had unexpected body: {:?}", resp_body)
        },
        StatusCode::NotFound => {
            if resp_body == CARD_NOT_FOUND_RESPONSE {
                println!("No card was found. This will be reflected in the output file.");
                let mut ret = K2Response { ..Default::default() };
        ...
        _ => panic!("Failed to get expected response from K2. Status: {:?}", status)
    }
}
    ...
    match ::reqwest::get(url) {
        Ok(ref mut resp) => {
            ...
            if let Some(content_header) = headers.get::<ContentType>() {
                if format!("{}/{}", content_header.type_(), content_header.subtype()) == "application/json" {
                    match resp.json() {
                        Ok(json) => json,
                        Err(_) => panic!("Unable to deserialize JSON response. Status: {:?}", resp.status())
                    }
                } else {
